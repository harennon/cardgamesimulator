...
- **LLD 100: Verify Tonk two-phase turn-timer re-arm + full-game / spectate / reconnect / join-by-code e2e coverage** — a verification-flavored LLD whose deliverable is browser-tier (Playwright) and socket-tier (integration) test evidence that the already-merged Tonk consumer chain plays end-to-end. No production source changed (the per-phase re-arm shipped in LLD 98 / PR #113 and stays game-agnostic — `MAX_AUTO_ACTIONS_PER_SEAT` is a generic per-seat cap, no Tonk special-casing was added to `TurnTimerService` or `socketHandler`).
