...
- **LLD 89: Tonk board rendering (read-only board)** — a new presentational `TonkBoard.vue` that renders the Tonk table entirely from the server-provided `TonkPublicState` + `you.hand`, dispatched by game type in `GameView.vue`. No action controls, no card selection, no emits — purely read-only (action controls are a later sub-issue). No engine, shared-type, socket, or state-composable changes; board selection happens only at the `GameView` component-dispatch layer.
  - `src/frontend/component/game/GameView.vue` — the only edit to existing code: a `v-if="gameState.gameType === 'tonk'"` branch renders `TonkBoard`, `v-else` renders `GameBoard`. Big2 props/emits and the final-play/game-over branches are unchanged.
  - `src/frontend/component/game/TonkBoard.vue` — the read-only board (A1 + B1 layout): reused `OpponentRow` + `RoomCodeChip` for opponent counts; a centered turn + phase banner ("{name}'s turn — discard/draw phase"); a three-slot table cluster (Stock face-down pile + count, Discard top card + count, and a visually distinct gold-dashed Drawable slot for `drawableDiscard` or a "No draw" placeholder when null); a persistent compact tallies strip (per-seat running score, lower-is-better, with the current player's row highlighted) plus the trick number; a non-interactive fanned hand strip; and `TonkGameLog`. Compact-seat CSS at >= 6 players and a mobile single-column grid with a teleported log drawer + compact inline tallies (approved direction).
  - `src/frontend/component/game-ui/TonkCardView.vue` — a thin Tonk card wrapper: face-down → card back; Joker (via the shared `isJoker` guard) → a gold joker glyph on a card face (never the literal word "Joker"); standard cards delegate to the existing `GameCard.vue`.
  - `src/frontend/component/game-ui/TonkGameLog.vue` — Tonk-shaped log (cannot reuse the Big2-typed `GameLog`): per-entry phrases ("discarded N×" with the discarded cards, "drew from {source}" with the drawn card never shown, "called TONK") plus a trick-result summary row (per-seat hand values + tally deltas). Mirrors `GameLog`'s auto-scroll-to-bottom.
  - `src/frontend/composables/useTonkBoard.ts` — pure read-only derivations the board renders from (tonkState, own hand, spectator detection, turn/phase labels, stock/discard/drawable, per-seat tally lookup, compact-seat threshold). Computes no game rules, legality, or scoring — only displays fields the engine already produced. Shared by the component and its tests so the tests assert the real derivation logic.
  - Tests (project pattern: node env, no DOM mount): `tests/frontend/tonkBoard.test.ts` (dispatch predicate; own-hand incl. Joker; discard top/count incl. empty pile; drawable distinct-from-top and null-placeholder; stock count incl. 0; turn + phase labels; per-seat tally alignment + length-mismatch defense; compact seating at 3/6/8; null/loading transition; spectator render + information-hiding — opponents carry counts only, stock has no card array), `tests/frontend/tonkGameLog.test.ts` (per-entry phrases, trick-result summary, drawn card never rendered, empty log), `tests/frontend/tonkCardView.test.ts` (back/joker/standard branch selection incl. mixed hands).
