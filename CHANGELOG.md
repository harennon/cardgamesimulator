...
- **LLD 68: Tonk game engine (pure `GameEngine`, fully unit-tested)** — a new server-authoritative, deterministic Tonk engine implementing the variant pinned in LLD 65. Action set is exactly `{ discard, draw, callTonk }`; a match is a sequence of tricks within one `InternalGameState` (status stays `IN_PROGRESS` across tricks, flips to `COMPLETED` only at match end). No melds/spreads/runs, no hitting, no drop/knock, no going-out-by-emptying-hand. The engine only **reads** `config.options.deckRoundsTarget` (default 8, clamped to [5,12]) and **populates** `breakdown.trueLoser`/`breakdown.lost`/`breakdown.finalTally`; the §8.8 cross-stack plumbing (API/DB/lobby) and the `StatsService` loss-centric derivation remain out of scope (#60 / LLD 66).
  - `src/shared/tonk-types.ts` — public shapes: `TonkJoker`/`TonkCard` (Tonk-local joker type; shared `Card`/`Rank` untouched), `isJoker`, `TonkTurnPhase`, `TonkDrawSource`, `TonkLogEntry`, `TonkPublicState`, and the `discard`/`draw`/`callTonk` action payloads.
  - `src/backend/engine/tonk/tonk-types.ts` — internal `TonkState` (hidden hands/stock, public discard pile, turn-start `drawableDiscard` snapshot, per-trick + per-match fields) and the `TonkAction` union.
  - `src/backend/engine/tonk/constants.ts` — `cardValue` (A=1, 2–10=face, J/Q/K=10, joker=0), `handValue`, `tonkCardEquals`, the stable `compareTonkCards` ordering (used by deterministic auto-timeout tie-break), `buildOrderedPool`, the §8.1 `cutAmount` formula, and `clampDeckRoundsTarget`.
  - `src/backend/engine/tonk/deck.ts` — `buildTonkDeck` (per-trick build → shuffle → blind cut → deal 5 each → stock; deterministic given a per-trick sub-seed) and `buildTrueLoserDeck` (always one fresh 54-card deck + 2 jokers, regardless of in-play `numDecks`).
  - `src/backend/engine/tonk/scoring.ts` — `scoreTrick` (Case A: TONK caller strictly lowest; Case B: caller tied/beaten; Case C: stock-out lowest-hand penalty, ties each +30), `resolveMatchEnd` (lost-seat collection + single-loser shortcut + deterministic TRUE-LOSER joker draw), and `finalScores` (numeric `lost`/`trueLoser`/`finalTally` flags; `score = finalTally`).
  - `src/backend/engine/tonk/valid-actions.ts` — `computeValidActions` (per-phase + TONK-gate action types), `validateDiscard` (non-empty, single-rank with jokers grouping only with jokers, all in-hand via exact instance match), `validateDrawSource` (stock always legal; discard only when `drawableDiscard !== null`; arbitrary source rejected).
  - `src/backend/engine/tonk/tonk-engine.ts` — `TonkEngine` implementing every `GameEngine` method: two-phase turn (`turnPhase`, hand-off only after the draw), turn-start `drawableDiscard` snapshot (never the live top, so no self-draw of a just-discarded card), inter-trick rebuild/cut + face-up start card + highest-tally starter, match-end resolution, and information-hiding views (own hand only; opponents and stock as counts). All inter-trick/end-game randomness uses derived sub-seeds (`:trick:N` / `:trueloser:N`) inside `applyAction` — no PRNG param, no `Math.random()`; `initialize` seeds the trick-1 deck from `randomSeed` identically so replay is exact.
  - `src/backend/engine/game-engine-factory.ts` — registers `new TonkEngine()` on the singleton factory.
  - Tests (`tests/engine/tonk/`): card/hand values; deck build + cut determinism asserting the §8.1 worked-example rows (3p default→15, target 12→3, target 5→24, target 13→0, 6p→30, 8p→4) and that the card SET differs across tricks at the default but is identical at the no-cut boundary; turn phases + valid actions; drawable-discard snapshot (buried-still-drawable, no self-draw, multiples, trick-2+ start card, recompute at hand-off); TONK gate + Cases A/B; stock-out Case C; match end + single/multiple-loser TRUE-LOSER draw + `winner` display tiebreak + loss-centric `breakdown`; invalid-action coverage (state + version unchanged); auto-timeout (highest card in discard phase with deterministic tie-break, never multiples/TONK; stock draw in draw phase; null when not active); information-hiding negatives; a full-match simulation asserting invariants every step (card conservation, valid seat, no deadlock, monotonic tallies, version +1) and that exactly one TRUE LOSER is derived from `breakdown.trueLoser`; and `engineFactory` registration for `"tonk"`.
