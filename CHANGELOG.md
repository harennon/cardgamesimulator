...
- **LLD 59: Tonk (Tunk) Rules Specification (docs-only — HARD GATE)** — added the approved rules specification document that pins down the exact Tonk variant to implement, so the downstream engine sub-issue (#41.2) can be authored with no rules TBDs. This is a documentation deliverable only: **no engine code was written**, per the LLD's explicit hard gate requiring user sign-off on the chosen variant before any engine work begins.
  - `docs/lld/59-tonk-rules-spec.md` — full rules spec: single 52-card deck, 2–4 players, 5-card deal; card point values (A=1, 2–10 face, J/Q/K=10); all win conditions (Tonk-on-deal at 49/50, going out, lowest-count at stock exhaustion, drop/knock with under-cut); spreads (sets 3–4 of a rank, runs 3+ same-suit ace-low no-wrap) and hitting onto any spread; drop/caught-drop scoring; multi-hand match to a target score (default 100) with `TONK_BONUS`/`UNDERCUT_PENALTY` of 25; draw→meld→discard turn structure; turn-timer auto-action semantics; information-hiding expectations; and a mapping of every rule onto the existing `GameEngine` interface and stats pipeline (no new engine plumbing). Flags three leaking-abstraction risks (two-phase turn vs single `getAutoTimeoutAction`, Tonk-on-deal producing `COMPLETED`, PRNG-free `applyAction` re-deals between hands) for the engine LLD to resolve deliberately.
  - **Blocked pending user sign-off** on the variant summarized in the doc's Variant Sign-Off table; engine implementation must not begin until that gate is cleared.
