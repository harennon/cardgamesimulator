...
- **LLD 149: game_history retention policy — prune unbounded append-only table**
  ...
  - `supabase/migrations/destructive-ddl.allowlist.json` — new file: allowlists `012_prune_game_history.sql: ["DELETE"]` (the single in-body `DELETE FROM game_history`); exactly one entry, reviewed and intentional.
  - `supabase/migrations/expected-diff.allowlist.json` — appends `012_prune_game_history.sql` to `expectedPending`.
  - `scripts/fixtures/clean-diff.json` — appends `012_prune_game_history.sql` to `pending` (mirrors `expectedPending` to satisfy drift-gate coupling invariant from PR #107).
