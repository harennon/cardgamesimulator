...
export class EchoHandler extends Handler {
    ...
    public override async put(request: Request<EchoRequest>, response: Response<EchoResponse>) {
        response.status(200).json(request.body)
    }

    public override async post(request: Request<EchoRequest>, response: Response<EchoResponse>) {
        response.status(200).json(request.body)
    }
}