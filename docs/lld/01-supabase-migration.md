...
export async function signUp(email: string, password: string, displayName: string): Promise<Session> {
  ...
  if (!data.session) throw new Error('Sign-up succeeded but no session returned');
  return data.session;
}
...
export interface GameRepository {
  createGame(gameId: string, gameType: GameType, creatorId: string, maxPlayers: number): Promise<Game>;
