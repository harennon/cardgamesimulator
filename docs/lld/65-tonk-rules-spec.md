...
**Status: GATE CLEARED — signed off by user on 2026-06-28 (see §9, Variant Sign-Off, for the recorded decisions). The docs-only HARD GATE has been satisfied; engine sub-issue #57 (and the §8.8 plumbing sub-issues) may now proceed against this spec.**
...
| Players | 3–5 with one deck; 6+ with two or more decks. **Signed off (2026-06-28): `minPlayers: 3`, `maxPlayers: 8`** — ≤5 = single deck (54 cards), 6–8 = two decks (`ceil(players/5)`); the pool is cut to the `deckRoundsTarget` (default 8), which cuts at every player count including ≤5 (see §8.1). Multi-deck is adopted, not deferred. |
...
4. **Loss-centric stats: exactly one loser (the TRUE LOSER); everyone else won.** (Signed off 2026-06-28; this reverses an earlier tally-vs-150 multi-loser model.) Tonk records **exactly one loss per game** — the TRUE LOSER (the joker-drawer, §5.3) — and **everyone else won, including players who also crossed 150**. Win/loss is derived per player from `breakdown.trueLoser`, **not** from `state.winner` and **not** from each player's tally-vs-150. The `StatsDelta` counters are already independent (verified: `database.ts:23-27`), so this is a *derivation-logic* change for Tonk, not a schema change, and it is **Tonk-specific** — Big2 has no `breakdown.trueLoser` and keeps its existing single-winner derivation. `state.winner` becomes a display-only "best result" value (lowest tally) that does **not** drive stats. Detail in §6.3.
...
- **Multi-deck (6+ players) in v1** — **adopted** (was previously deferred; that deferral is reversed here). It adds modest deck-multiplicity bookkeeping (`numDecks = ceil(players / 5)`), but the rest of the stack already supports it: the backend has **no hard player cap** (`createGame.ts` passes `maxPlayers` through with only a truthy check; engines self-validate via `GameEngineConfig.minPlayers/maxPlayers` — verified), and the frontend `OpponentRow.vue` is an uncapped `v-for` flex row that renders any count (6+ is only visually cramped — CSS polish, not a blocker). **Signed off (2026-06-28): `maxPlayers: 8`** (the user chose 8, above the ruleset's 6-player single→multi-deck cutoff). ≤5 stays single-deck, 6–8 triggers two decks; both are cut to the `deckRoundsTarget` (default 8 cuts at all counts — §8.1, §9.9). The TRUE-LOSER draw uses a single regular deck + 2 Jokers regardless of in-play deck count (§5.3, §8.5). **(§9.1)**
...
| `winner` | Set at `COMPLETED` to the player with the lowest final tally (ties → lowest seat index); **display/best-result only; does NOT drive stats — see §6.3.** Stats derive from `breakdown.trueLoser` (only the TRUE LOSER lost), not from this field. |
| `scores` | At `COMPLETED`: one `PlayerScore` per player. `score` = final running tally (lower is better). `breakdown` carries `{ lost: 0|1, trueLoser: 0|1, finalTally }`, where `trueLoser = 1` on exactly the TRUE LOSER (§5.3) and **this — not `winner`, not `lost` — is what drives `gamesWon`/`gamesLost`** (§6.3): the TRUE LOSER gets `gamesLost: 1`, everyone else `gamesWon: 1`. `lost = (finalTally >= 150) ? 1 : 0` is retained as informational/flavor (who crossed 150) but does NOT drive win/loss. **`PlayerScore.breakdown` is typed `Record<string, number>` (verified: `engine-types.ts:80`), so `lost`/`trueLoser` are numeric `0\|1` flags, NOT booleans — #57 must encode them as numbers (e.g. `trueLoser: 1`), not `true`/`false`.** |
...
- If **more than one** player has lost: shuffle a **single fresh standard deck (52 cards + 2 Jokers = 54), regardless of how many decks were in play** during the match, via the PRNG, then the lost players draw one card at a time in seat order, repeating, until a **Joker** is drawn. The player who draws the Joker is the **TRUE LOSER**. (Deterministic given seed; §8.5.) With 2 Jokers in 54, termination is guaranteed. (Signed off 2026-06-28: the draw deck is always one deck + 2 Jokers — it is NOT scaled to the in-play `numDecks`.)
...
> **Forward reference (out of scope here):** Game-specific (per-game-type) stats are specified in a separate forthcoming stats LLD (LLD 66). This section assumes the existing **global** `player_stats` contract and changes only the win/loss **derivation** for Tonk. **Tonk also needs a meaningful average-score stat** (`totalScore / gamesPlayed`) — which is only meaningful once stats are segmented per game type (LLD 66), because Tonk's lower-is-better penalty scores would otherwise be averaged against Big2's higher-is-better scores. That segmentation is LLD 66's job; this LLD only flags the requirement.

**Loss-centric model (signed off 2026-06-28).** Tonk records **exactly ONE loser per game — the TRUE LOSER** (the joker-drawer, §5.3) — and **everyone else won, including any player who also crossed 150.** (This was resolved by the user against an earlier tally-vs-150 model that would have marked every ≥150 player a loser; see the §9 sign-off, items 6/8.) Win/loss is driven by `breakdown.trueLoser`, **not** by `state.winner` and **not** by each player's tally-vs-150.

**Counters are already independent — only the derivation changes.** `StatsDelta` (`src/backend/database/database.ts:23-27`) has **independent integer counters**: `gamesWon: number` (line 25), `gamesLost: number` (line 26), `totalScore: number` (line 27). They are **not** mutually derived in the type, so this mapping needs **no change to `StatsDelta` or the DB schema** — only the derivation logic changes.

`StatsService.recordGameCompletion` (`src/backend/service/statsService.ts:18-31`) **today** reads `const winnerId = state.winner;` (line 22) and derives, per non-guest player, `gamesWon: playerScore.playerId === winnerId ? 1 : 0` (line 29) and `gamesLost: playerScore.playerId !== winnerId ? 1 : 0` (line 30). That is the **single-winner** derivation. For Tonk this must be **decoupled from `state.winner`** and driven by `breakdown.trueLoser` instead.

**Derivation for Tonk (per player at `COMPLETED`, driven by `breakdown.trueLoser`, NOT `state.winner` and NOT tally-vs-150):**
- `gamesLost = (breakdown.trueLoser === 1) ? 1 : 0` — **only** the TRUE LOSER loses. Exactly one loser per game.
- `gamesWon  = (breakdown.trueLoser === 1) ? 0 : 1` — everyone else won, even a player who also crossed 150.
- `totalScore = finalTally` (still lower-is-better — note this inverts Big2's "higher score is better"; documented here so leaderboard/`totalScore` aggregation understands Tonk scores are **penalties, not achievements**, and so the average-score stat is interpreted correctly once LLD 66 segments by game type).

**This must be Tonk-specific so Big2 is unaffected.** The clean, additive, backward-compatible approach: `StatsService` reads `breakdown.trueLoser` **when present** and uses the loss-centric derivation above; when absent (Big2), it keeps the existing single-winner derivation (`state.winner`). Big2 never populates `breakdown.trueLoser`, so its behavior is unchanged. The engine (#57) MUST populate `breakdown.trueLoser` (1 on the TRUE LOSER, 0 on everyone else) so this derivation has its input. The small `StatsService` change (read `breakdown.trueLoser` when present) belongs to the stats sub-issue / LLD 66, not the engine. `state.winner` is decoupled — it is a display-only "best result" value (§4.1, lowest tally) and does not drive stats.
...
- Uses a **single fresh standard deck: 52 cards + 2 Jokers = 54, ALWAYS** — regardless of how many decks (`numDecks`) were in play during the match. (Signed off 2026-06-28: the draw deck is NOT scaled to `numDecks`; one deck + 2 Jokers is simpler and sufficient.) Shuffled via sub-seed `hashSeed(randomSeed + ":trueloser:" + trickNumber)`.
- Lost players draw in ascending seat order, looping, until a Joker is drawn; that player is TRUE LOSER. With 2 Jokers in 54, termination is guaranteed. Single lost player → automatic TRUE LOSER (no draw).
...
| Initialize with <3 or >8 players | `initialize` throws `"Tonk requires 3-8 players"` (§9.1). |
...
> **SUPERSEDED (persistence only) by LLD 95 (`docs/lld/95-deck-rounds-target-game-config.md`).** Step (c) below originally prescribed a **dedicated `deck_rounds_target` INT column** on `games` (and a parallel `SerializableGame.deckRoundsTarget: number | null`). The product-owner "Restart" decision on #60 rejected that. Persistence is now **one generic, typed JSONB column** `games.game_config` (migration `009_add_game_config.sql`): Big2 persists `{}`, Tonk persists `{"deckRoundsTarget": <n>}`, with typing at the application boundary via the `GameConfig` interface in `src/shared/model.ts`. The client API shape is unchanged (flat, top-level `CreateGameRequest.deckRoundsTarget?: number`), validation `[5,12]` in `createGame.ts` is unchanged, the §9.9 sign-off of "build the creator-config plumbing" still holds, and the engine contract (read `config.options.deckRoundsTarget`, clamp/default 8) is unchanged. Only the **storage shape** moved from a dedicated column to the `game_config` JSONB blob. Steps (a), (b), (d) below are accurate; step (c) is replaced by the `game_config` JSONB column.
...
## 9. Variant Sign-Off (HARD GATE — CLEARED 2026-06-28)

> ### ✅ SIGNED OFF BY USER — 2026-06-28
>
> The user reviewed the variant and confirmed all items below directly. The docs-only HARD GATE is **CLEARED**; engine sub-issue #57 and the §8.8 plumbing sub-issues may proceed. Decisions as confirmed, with **overrides from the proposed defaults called out**:
>
> 1. **§9.1 — Player range = 3–8** (OVERRIDE: user chose `maxPlayers: 8`, not the proposed 6). `numDecks = ceil(players/5)`: 3–5 → 1 deck, 6–10 → 2 decks (so 6–8 → 2 decks). 6–8 frontend seating is non-blocking CSS polish.
> 2. **§9.2 — Deck-size heuristic confirmed:** default `deckRoundsTarget = 8`, deterministic blind cut.
> 3. **§9.3 — Trick-1 starter = seat 0** (game creator). Confirmed.
> 4. **§9.4 — Timeout auto-action confirmed:** discard-phase → single highest card (never multiples/TONK); draw-phase → stock; timer re-arms per phase (hard LLD-07 integration requirement, §10).
> 5. **§9.5 — TRUE-LOSER draw uses a SINGLE regular deck + 2 Jokers, ALWAYS** (OVERRIDE: no longer scaled to `numDecks`). In-play decks still naturally contain `2 * numDecks` Jokers as 0-value cards; only the end-of-game draw deck is pinned to one deck + 2 Jokers.
> 6. **§9.6 / §9.8 — LOSS-CENTRIC stats** (OVERRIDE: reverses the earlier tally-vs-150 multi-loser model). **Exactly ONE loser per game — the TRUE LOSER** (joker-drawer); **every other player won, including players who also crossed 150.** `gamesLost` derives from `breakdown.trueLoser`, Tonk-specific so Big2's single-winner derivation is untouched. The all-lost edge is therefore no longer special (still exactly one loser). Tonk also wants **average score** as a meaningful stat — which requires the game-specific stats segmentation (separate stats LLD), since Big2's higher-is-better scores would pollute a global average.
> 7. **§9.7 — Case-C stock-out tie:** each tied-lowest player adds 30. Confirmed.
> 8. **§9.8 — All-lost edge:** resolved by item 6 (only the TRUE LOSER loses); `state.winner` stays the lowest-tally display value.
> 9. **§9.9 — BUILD the full `deckRoundsTarget` creator-config plumbing** (OVERRIDE of the "internal constant" alternative): typed `deckRoundsTarget?: number` field + range validation + a NEW `games`-table column + `gameService` wiring + lobby control. Default 8; cutting at ≤5 players accepted. NOTE: this `games`-table migration is SEPARATE from the game-specific-stats LLD's `player_stats` migrations (different table, different PR).
>
> The records below the line are the original pre-sign-off proposal text, retained for provenance; where they say "(default — confirm)" or "proposed," read the confirmed decision above as authoritative.

This spec is transcribed from the user's authoritative `TONK.Rules.md` (verified faithful); the items below were engine-critical points the ruleset's prose left open, resolved with concrete defaults and now confirmed by the user (see the sign-off block above for overrides).

Confirmed items (original proposal text retained for provenance):

1. **Player range = 3–8.** ✅ **Confirmed 2026-06-28 (OVERRIDE: user chose 8, not the proposed 6).** ≤5 players → 1 deck (52 + 2 Jokers = 54); 6–8 players → `ceil(players/5)` = 2 decks shuffled together. The pool is cut to the `deckRoundsTarget` (default 8), which cuts at **every** player count including ≤5 (§8.1, §9.9) — the card set changes between tricks at all counts. Multi-deck adopted (reverses the earlier deferral). 6–8 frontend seating is a downstream **non-blocking** CSS-polish cost (§2.3).
2. **Deck-size heuristic:** ✅ **Confirmed 2026-06-28.** `numDecks = ceil(players/5)`; the cut amount comes from the unified §8.1 formula driven by `deckRoundsTarget` (creator-set, range 5–12, default 8 — §9.9), deterministic blind cut via seed; Jokers may be cut from a trick's deck. At the default target the cut applies at all player counts (e.g. 15 cards at 3 players); `cutAmount = 0` only when a high enough target makes `targetCards ≥ poolSize` (`deckRoundsTarget ≥ 13` for 3 players — above the default).
3. **Trick-1 starter = seat 0** (ruleset specifies the starter only for trick 2+). ✅ **Confirmed 2026-06-28.**
4. **Turn timer auto-action:** discard-phase → discard single highest card (#57 defines a deterministic stable tie-break order, §7; never auto-TONK); draw-phase → draw from stock. Timer re-arms per phase — a **hard cross-LLD integration requirement** (LLD 07), tracked in §10, not merely a preference: without per-phase re-arm a timed-out turn stalls (§7). ✅ **Confirmed 2026-06-28.**
5. **Joker = value 0 and the TRUE-LOSER token.** ✅ **Confirmed 2026-06-28 (OVERRIDE: TRUE-LOSER draw deck simplified).** In-play decks contain `2 * numDecks` Jokers as 0-value cards. The **TRUE-LOSER draw always uses a single regular deck + 2 Jokers** (NOT scaled to `numDecks`) — §5.3, §8.5.
6. **Stats mapping (loss-centric):** ✅ **Confirmed 2026-06-28 (OVERRIDE: reverses the earlier tally-vs-150 model).** **Exactly ONE loser per game — the TRUE LOSER**; everyone else won, including players who also crossed 150. `gamesLost = (breakdown.trueLoser === 1) ? 1 : 0`, `gamesWon` the complement; `totalScore = finalTally` (penalty, lower is better). Driven by `breakdown.trueLoser`, NOT by tally-vs-150 and NOT by `state.winner` (which stays display-only, lowest tally). Tonk-specific so Big2 is unaffected (Big2 has no `breakdown.trueLoser`); a small additive `StatsService` change reads `breakdown.trueLoser` when present. Derivation-logic change, NOT a `StatsDelta`/DB-schema change (counters already independent — `database.ts:23-27`). Tonk additionally wants an **average-score** stat, which needs the per-game-type segmentation in the separate stats LLD (LLD 66, §6.3).
7. **Case C tie (multiple lowest hands at stock-out):** each tied-lowest player adds 30. ✅ **Confirmed 2026-06-28.**
8. **All-players-lost edge:** ✅ **Confirmed 2026-06-28 (resolved by item 6).** Even if all players cross 150, there is still exactly ONE loser (the TRUE LOSER by joker draw); everyone else gets `gamesWon: 1`. `winner` = lowest tally among them (display-only). No `null`-winner special case needed.
9. **Creator-configurable deck cut (`deckRoundsTarget`) — BUILD the full plumbing.** ✅ **Confirmed 2026-06-28 (OVERRIDE: user chose to build it, not keep it an internal constant).** The game creator picks `deckRoundsTarget` in the lobby (control type: a **number input / slider**, **range 5–12, default 8**), driving the §8.1 cut formula: a low value cuts more, a high value (≥13 for 3 players) yields no cut.

   - **Default-cut sub-decision — CONFIRMED.** Keep `deckRoundsTarget` default = 8 and **accept the resulting cut at ≤5 players** (e.g. 15 cards at 3 players), so the deck changes between tricks even at low player counts. §8.1 prose and examples reflect this. The no-cut boundary (`deckRoundsTarget ≥ 13` for 3 players) remains available to a creator who explicitly wants no cut, but is not the default.
   - **Scope-expansion decision — CONFIRMED: build the plumbing.** The full creator-control path (§8.8) is **in scope**: `deckRoundsTarget?: number` on `CreateGameRequest` (model.ts) as a dedicated typed field (NOT reusing the dead `gameOptions` bag — §8.8), range validation (5–12) in createGame.ts, a NEW persisted column on the `Game` entity, the `gameService.startGame` change to pass it into `config.options` instead of the hardcoded `{}` (`gameService.ts:102`), and a lobby control mirroring the turn-timer picker. These are distinct sub-issues from the engine (#57). **NOTE:** this `games`-table migration is SEPARATE from the game-specific-stats LLD's (LLD 66) `player_stats` migrations — different tables, different PRs; do not conflate them.

The decisions above are recorded in the dated sign-off block at the top of this section. The gate is **CLEARED** as of 2026-06-28; engine sub-issue #57 and the §8.8 plumbing sub-issues may proceed.
...
| `src/backend/service/statsService.ts` | Implemented (single-winner derivation today, `statsService.ts:22/29-30`) | Loss-centric stats mapping §6.3: small additive change to read `breakdown.trueLoser` when present (Tonk-specific; Big2 unaffected). Belongs to the stats sub-issue / LLD 66, not the engine. |
...
**This LLD has no code dependencies — it is docs-only.** It is **signed off** (§9, 2026-06-28), so downstream sub-issue #57 (engine) may proceed. **The §8.8 creator-config plumbing (`deckRoundsTarget`) is a SCOPE EXPANSION beyond #57 that the user confirmed BUILDING (§9.9)** — it spans the four rows above (API + DB + frontend) as distinct sub-issues from the engine.
...
- **Exactly one loser:** the TRUE LOSER gets `gamesLost: 1, gamesWon: 0`; **every other player** gets `gamesWon: 1, gamesLost: 0` — including any player who also crossed 150.
- **Crossing 150 does NOT by itself record a loss:** a player at ≥150 who is not the TRUE LOSER still gets `gamesWon: 1`.
- **All-lost game:** if every player is ≥150, still exactly one loser (the TRUE LOSER); everyone else gets `gamesWon: 1`.
- **Derivation key is `breakdown.trueLoser`:** `gamesLost = (breakdown.trueLoser === 1) ? 1 : 0`; it is set to 1 on exactly the TRUE LOSER and 0 on everyone else.
- **Big2 unaffected:** a game with no `breakdown.trueLoser` (Big2) keeps the existing single-winner derivation.
- `totalScore` = final tally (penalty, lower is better).
- `state.winner` (lowest tally) is a display value and does **not** drive any of the above.
...
- Assert invariants hold every step, the match terminates (no infinite loop), and `winner`/`scores`/`trueLoser` are populated (with `winner` as the display lowest-tally value and `scores[].breakdown.lost`/`trueLoser` set). Assert the loss-centric stats derivation consumes the result correctly: the TRUE LOSER gets `gamesLost: 1, gamesWon: 0` and every other player gets `gamesWon: 1, gamesLost: 0` (including any player ≥150 who is not the TRUE LOSER), derived from `breakdown.trueLoser` (not `winner`, not tally-vs-150); `StatsService.recordGameCompletion` runs without error (per §6.3 mapping).
