# LLD 64: Tonk Rules Specification
...
This LLD pins down the EXACT Tonk (Tunk) variant we will implement. Tonk has many regional variants; choosing wrong poisons every downstream sub-issue (engine #57, frontend, etc.). This spec is authored to **exactly** match the user's authoritative `TONK.Rules.md` (attached to issue #56 on 2026-06-28: *"Do not deviate from this rule set"*).

> **Supersedes PR #73 / LLD 59.** That spec describes a fundamentally different game (52-card no-joker deck, 5-card deal but with **spreads, runs, melds, hitting, drop/knock with under-cut**, draw-then-discard, single-hand winner-takes points). The user's ruleset has **none of those mechanics**. PR #73 must be discarded or fully rewritten; do not adopt its variant. Where this doc and PR #73 disagree, **this doc is authoritative.**
...
- Turn structure: **discard-first, then draw** (the reverse of normal rummy/Tonk and the reverse of PR #73).
...
- **Going out by emptying the hand.** Hands never empty — every turn discards then draws back to the same size band; you win a trick by *calling TONK*, not by running out of cards.
...
| Players | 3–5 with one deck; 6+ with two or more decks. **We support `minPlayers: 3`, `maxPlayers: 5` for v1** (single-deck only — see §8.1 for why multi-deck is deferred, presented as a default for sign-off). |
...
| Draw source | Round 1 leader: stock only (trick 1) / stock or the trick-start face-up discard (trick 2+). From the 2nd player onward each turn: stock **OR** the single most-recent card discarded by the immediately-preceding player. If that player discarded multiples, only 1 may be taken. |
...
4. **Lower-is-better maps onto the winner-centric stats pipeline via a documented adapter, not a pipeline change.** `StatsService.recordGameCompletion` derives win/loss from `state.winner` + `state.scores`. We keep that contract: at game end we set `state.winner = <the player who did NOT lose / lowest tally>` and record the **TRUE LOSER** as the one player who took the loss. This is a *leaking-abstraction risk* and is called out explicitly in §6.3 — but the simplest mapping is to populate the existing fields correctly rather than to change generic plumbing.
...
- **Multi-deck (6+ players) in v1** — rejected for v1. Adds deck-multiplicity bookkeeping and changes the Joker-count math for the TRUE LOSER draw, for a player count the rest of the stack (lobby max, frontend seating) is not yet built for. Deferred; `maxPlayers` cap of 5 keeps us single-deck. **Recommended default — confirm at sign-off (§9).**
...
4. **Discard pile.**
   ...
   - **Trick 2+:** flip **1** card face up from the stock into the discard pile before play starts (gives the trick starter a draw option).
5. **First player.**
   - **Trick 1:** the player at seat index 0 (game creator's seat / lobby order) starts. (The ruleset does not specify trick-1 starter; default to seat 0 — confirm at sign-off, §9.)
   - **Trick 2+:** the player with the **overall highest tally** starts (and is the dealer). Ties broken by lowest seat index (§8.4).
...
**Hand value** = sum of the values of all cards in the hand.
...
2. **Draw phase.** The player draws exactly **1** card and adds it to their hand:
   ...
   - **Every other turn (2nd player onward, and all turns trick 2+):** from the **stock**, OR the single most-recent card on top of the discard pile that was placed by the **immediately preceding** player. If that preceding player discarded multiples, only the **top 1** may be taken.

Hand size returns to its pre-turn count each turn (discard *n*, draw 1 → if you discarded 1 you stay at 5; if you discarded multiples you shrink). Hands therefore monotonically shrink or stay equal across a trick; this is fine — winning is by TONK, not by emptying.
...
- **Gate:** TONK may only be called after **every player has had at least one full turn** (i.e. from the start of the 2nd round onward). Tracked via `everyoneHadATurn` / `trickTurnCount` in state (§4.2).
- On TONK: all hands are revealed and the trick ends immediately. Scoring per §5.1.

### 3.5 Stock exhaustion

If the stock empties (a player must draw but the stock has 0 cards) before any TONK is called, the trick ends: all hands revealed, lowest hand takes the penalty (§5.1). See §7 for the exact mid-turn timing.
...
| `winner` | Set at `COMPLETED` to the player who did **not** lose / has the lowest final tally (stats adapter, §6.3). |
...
| `log: TonkLogEntry[]` | Public action history (discards, draws-source, TONK calls, trick results). |
...
- **Reset:** `hands`, `stock`, `discardPile`, `lastDiscard*`, `turnPhase` → `"discard"`, `trickTurnCount` → 0, `tonkCallerIndex` → null. New deck is rebuilt and cut deterministically (next sub-seed derived from `randomSeed` + `trickNumber`, §8.1).
...
- All other players add **0**.
- Ties for lowest in Case C: see §7 (all tied-lowest players each add 30 — default, confirm at sign-off §9).
...
Every method of `GameEngine` (`src/backend/engine/game-engine.ts`) maps as follows. Items flagged ⚠ are leaking-abstraction risks surfaced now per the issue's instruction.
...
| `applyAction(state, action)` | Dispatch on `action.type ∈ {discard, draw, callTonk}` and `turnPhase`. Deterministic; no PRNG param. **All mid-trick and inter-trick randomness (new deck cut, TRUE-LOSER draw) uses a PRNG re-seeded deterministically from `randomSeed` — see ⚠-A.** Returns immutable new state, `version + 1`. |
...
`StatsService.recordGameCompletion` (`src/backend/service/statsService.ts`) derives, per player: `gamesWon = (playerId === state.winner) ? 1 : 0`, `gamesLost = (playerId !== state.winner) ? 1 : 0`, `totalScore = playerScore.score`. It is **winner-centric and single-winner** (exactly one winner, everyone else "lost").
...
**The `gamesLost` semantics mismatch is the residual risk.** With the recommended mapping, the existing `StatsService` would record `gamesLost: 1` for **every** player except the lowest-tally winner — which over-counts losses for Tonk (only the TRUE LOSER truly "lost"). Two options for #57 / the stats sub-issue to choose between (decision deferred to the stats sub-issue, **not** this spec — flagged here so it is not missed):
...
⚠ **Leaking-abstraction note (randomness in `applyAction`).** The interface says `applyAction` "takes no PRNG — it must be deterministic" and "mid-game randomness must use a pre-shuffled deck stored in `gameSpecificState`." Big2 needs no mid-game randomness. Tonk needs randomness **between tricks** (rebuild + cut a new deck) and **at game end** (TRUE-LOSER joker draw). We satisfy the determinism contract by re-seeding a `SeededPRNG` from a **derived sub-seed** (`hash(randomSeed + ":trick:" + trickNumber)` / `+ ":trueloser"`) inside `applyAction`. Same `(state, action)` → same result, no external PRNG needed. This maps cleanly with **no interface change**; flagged so #57 implements the sub-seed scheme rather than reaching for `Math.random()` or threading a PRNG param.
...
- **The cut:** shuffle the 54-card deck with the PRNG, then **remove the top `(54 - targetCards)` cards** (the "cut"), keeping at least both Jokers? **No** — to honor "cards used changes every trick," the cut is a blind cut: cards removed may include Jokers. This is intentional and deterministic. (If a trick's deck ends up with 0 Jokers it has no effect on that trick — Jokers only matter for the end-of-game draw, which uses a **fresh full deck**, §8.5.)
- **Override:** `config.options.deckTargetRounds` (number) and `config.options.extraDecks` (number, default 0) allow tuning; v1 lobby may not expose these (defaults used). `GameEngineConfig.options` is already `Record<string, unknown>` — no plumbing change.
- **Determinism:** the per-trick deck uses sub-seed `hash(randomSeed + ":trick:" + trickNumber)`; same seed + trick → identical deck and cut (testing-principles #2; can be disabled/fixed via `FixedPRNG`).
...
- `draw` from `"discard"` is legal **only** when `lastDiscardPlayerIndex` is the immediately-preceding active player **and** the discard pile is non-empty.
...
- Uses a **fresh full deck** (52 + 2 Jokers), shuffled via sub-seed `hash(randomSeed + ":trueloser:" + trickNumber)`.
...
| Tie for lowest in Case C | All tied-lowest players each add 30 (default — confirm §9). |
...
| Player ≥150 but also lowest tally | Still "lost" (any ≥150 has lost, per ruleset); cannot be the `winner`. If *all* players are ≥150, `winner` = lowest tally among them but they are still all "lost"; TRUE LOSER decides. (Confirm at §9 whether `winner` should be null in an all-lost game.) |
...
**Engine sub-issue #57 MUST NOT begin until the user explicitly signs off on the points below.** This spec is transcribed from the user's authoritative `TONK.Rules.md`; the items marked **(default)** are engine-critical points the ruleset's prose left open, resolved here with a concrete default for confirmation.
...
| --- | --- | --- |
| `src/backend/engine/game-engine.ts` (`GameEngine`) | Implemented (LLD 02) | Interface the Tonk engine (#57) implements. |
| `src/shared/engine-types.ts` | Implemented | `InternalGameState`, `PlayerView`, `Card`, etc. **Joker gap flagged §8.6** (resolved Tonk-locally in #57). |
| `src/backend/engine/prng.ts` (`PRNG`, `SeededPRNG`, `FixedPRNG`, `hashSeed`) | Implemented | Deterministic deck cut + TRUE-LOSER draw via sub-seeds. |
| `src/backend/engine/game-engine-factory.ts` | Implemented | `TonkEngine` registered here in #57. `"tonk"` already in `GameType`. |
| `src/backend/service/statsService.ts` | Implemented | Stats mapping §6.3 (recommend isolated additive change, option 2). |
